...
export const vetInsightStagingConfig = (): Configuration => {
  const vpcStackReference = new StackReference("vpc-vetinsight-staging")
  ...
  return {
    ...
    privateSubnetIds,
    envName: "staging",
    eksClusterName: "vetinsight-staging",
    ingress: {
      public: {
    ...
    k8sClusterAdminArns: {
      roles: k8sRoleArnsFor({ account: "vetInsight", env: "staging", clusterPermission: "clusterAdmin" }),
      users: [],
    },
    k8sViewOnlyArns: {
      roles: k8sRoleArnsFor({ account: "vetInsight", env: "staging", clusterPermission: "viewOnly" }),
      users: [],
    },
    k8sNamespaces: [
        {
